...
class db {
	...
	function __construct() {
		...
		// 设置数据库名
		$this->db_name = globalVariables::$dance_db;
		
		// 读取数据库用户名和密匙
		include_once(globalVariables::$config_file_path);
		$this->manager = new MongoDB\Driver\Manager("mongodb://".authDance::$MongoDBUserName4Wxmini.":".authDance::$MongoDBUserPwd4Wxmini."@localhost:27017");
	}
	...
	*/
     ...
     * 保存文章到数据库
     */
    function saveArticle($doc_article, $collection_name) {
        $db = get_db();
        $doc_article = array_merge($doc_article, ['_id' => $db->insert($collection_name, $doc_article)]);
        return $doc_article;
    }
